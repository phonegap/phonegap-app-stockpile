...
describe('Favorites.vue', () => {
  ...
  it('should have a name of "Favorites"', () => {
    expect(Favorites.name).to.equal('Favorites');
  });

  describe('Data', () => {
    it('should have a data method', () => {
      expect(Favorites.data).to.be.a('function');
    });
    it('should have a data method that returns a title of "Favorites Page"', () => {
      expect(Favorites.data().title).to.equal('Favorites Page');
    });
    it('should have a content-block-title that displays `data().title`', () => {
      expect(vm.$el.querySelector('.content-block-title').textContent)
        .to.equal(Favorites.data().title);
    });
  });
});
